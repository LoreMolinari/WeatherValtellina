...
class MeteoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        meteoDomani.setOnClickListener(){
            val i = Intent(applicationContext, com.dinatium.weatherValtellina.meteoDomani::class.java)
    ...
    inner class weatherTask() : AsyncTask<String, Void, String>() {
        ...
        override fun doInBackground(vararg params: String?): String? {
            ...
            try{
                if(place.toString().equals("Valfurva")){
                    response = URL("https://api.openweathermap.org/data/2.5/weather?lat=46.414303131132186&lon=10.491103526898536&appid=bad9662e97abd6a3ab5f9dbd245f943c&lang=it&units=metric").readText(Charsets.UTF_8)
                }else{
                    response = URL("https://api.openweathermap.org/data/2.5/weather?q=$place&appid=bad9662e97abd6a3ab5f9dbd245f943c&lang=it&units=metric").readText(Charsets.UTF_8)
                }
            ...
            val icon = JSONObject(response).getJSONArray("weather").getJSONObject(0).getString("icon")
        ...
        override fun onPostExecute(result: String?) {
            try {
                ...
                findViewById<TextView>(R.id.sunrise).text = SimpleDateFormat("HH:mm", Locale.ITALIAN).format(Date(sunrise * 1000))
                findViewById<TextView>(R.id.sunset).text = SimpleDateFormat("HH:mm", Locale.ITALIAN).format(Date(sunset * 1000))
